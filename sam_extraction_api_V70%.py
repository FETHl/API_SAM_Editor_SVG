...
from flask import Flask, request, jsonify, send_file, redirect, send_from_directory, Response
...
app.config.update(
    ...
    EXPORTS_FOLDER=os.environ.get('EXPORTS_FOLDER', 'exports'),  # New folder for exports
...
for folder in [
    ...
    app.config['EXPORTS_FOLDER']  # Create the exports folder
]:
...
class SAMExtractor:
    ...
    def __init__(self, sam_checkpoint=None, model_type="vit_h", device=None):
        ...
        self.date = "2025-05-29 22:12:21"  # Updated timestamp
    ...
    def load_sam_model(self):
        ...
        try:
            ...
            model=sam,
            points_per_side=64,                    # Augmente la densité de points => plus de détails
            points_per_batch=128,                  # Plus grand batch => plus rapide (attention à la RAM)
            pred_iou_thresh=0.60,                  # Plus bas => accepte plus de masques, même avec moins de confiance
            stability_score_thresh=0.7,           # Plus bas => tolère plus de variations => + de masques
            crop_n_layers=1,                       # Active le recadrage multi-échelle (améliore la précision sur petits objets)
            crop_n_points_downscale_factor=1,      # Maintient la densité de points même après crop
            min_mask_region_area=0                 # Supprime le filtre de petite taille => garde tous les masques
             )
    ...
    def refine_edges(self, image, mask):
        ...
        # Convert mask to uint8 binary format (0 or 1) - FIXED FOR CRF
        binary_mask = (mask > 0).astype(np.uint8)
        
        # CRITICAL FIX: Ensure mask only contains 0 and 1 values
        # The CRF error was caused by mask having 255 values instead of 0/1
        binary_mask[binary_mask > 1] = 1  # Clamp any value > 1 to 1
    ...
    def _apply_crf(self, image, mask, iterations=5):
        ...
        try:
            ...
            # CRITICAL FIX: Ensure the mask only contains 0 and 1 values
            mask_labels = np.zeros_like(mask, dtype=np.int32)
            mask_labels[mask > 0] = 1
            
            # Double-check that we only have 0 and 1
            unique_values = np.unique(mask_labels)
            if np.any(unique_values > 1) or np.any(unique_values < 0):
                logger.error(f"Invalid mask values: {unique_values}. Expected only 0 and 1.")
                # Force clamp to 0 and 1
                mask_labels = np.clip(mask_labels, 0, 1)
                unique_values = np.unique(mask_labels)
            
            logger.debug(f"Mask labels min: {mask_labels.min()}, max: {mask_labels.max()}, unique: {unique_values}")
            
            # Create CRF with the image dimensions
            d = dcrf.DenseCRF2D(w, h, 2)  # 2 labels: fg and bg
            
            # Create unary potentials from the mask - FIXED CALL
            U = unary_from_labels(mask_labels, 2, gt_prob=0.7, zero_unsure=True)
            d.setUnaryEnergy(U)
...
def generate_svg(contours, width, height, contour_names=None):
    """
    Generate an SVG representation of the contours
    
    Args:
        contours: List of contour objects
        width: Image width
        height: Image height
        contour_names: Optional dictionary mapping contour IDs to names
    """
    # Create SVG element
    svg = ET.Element('svg')
    ...
    desc.text = f"SAM-extracted contours - Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    
    # Sort contours by area (largest first)
    sorted_contours = sorted(contours, key=lambda x: x.get('area', 0), reverse=True)
    
    # Create a group for all contours
    g_elem = ET.SubElement(svg, 'g')
    g_elem.set('id', 'contours')
    
    for contour_data in sorted_contours:
        points = contour_data.get('points', [])
        is_external = contour_data.get('is_external', True)
        contour_id = contour_data.get('id', -1)
        
        if len(points) < 3:
            continue
        
        # Get color for this contour
        if 'color' in contour_data:
            # Convert from 0-1 float RGB to hex
            r, g, b = contour_data['color']
            color = f"#{int(r * 255):02x}{int(g * 255):02x}{int(b * 255):02x}"
        else:
            color = "#00FF00"  # Default green
        
        # Create a path for this contour
        path = ET.SubElement(g_elem, 'path')
        path.set('id', f"contour_{contour_id}")
        
        # Create a title element for the contour name
        title = ET.SubElement(path, 'title')
        if contour_names and str(contour_id) in contour_names:
            title.text = contour_names[str(contour_id)]
        else:
            title.text = f"Contour {contour_id}"
        
        # Create SVG path data - ensure we have valid coordinates
        path_data = []
        if points and len(points) >= 3:
            path_data.append(f"M {points[0][0]},{points[0][1]}")
            
            for i in range(1, len(points)):
                path_data.append(f"L {points[i][0]},{points[i][1]}")
            
            path_data.append("Z")  # Close the path
            
            path.set('d', " ".join(path_data))
            path.set('fill', color if is_external else 'none')
            path.set('stroke', color)
            path.set('stroke-width', '2' if is_external else '1')
            path.set('fill-opacity', '0.3' if is_external else '0')
            path.set('stroke-opacity', '1')
    
    return ET.tostring(svg, encoding='unicode')

def generate_png_from_contours(contours, width, height, transparent=False, overlay_image=None):
    """
    Generate a PNG image from contours
    ...
    Args:
        ...
        overlay_image: Optional background image to overlay contours on
    """
    # Create base image
    if overlay_image is not None:
        # Use provided overlay image
    ...
    for contour_data in contours:
        ...
        # Draw filled contour if external, outline if internal
        if is_external:
            cv2.fillPoly(img, [points_array], color)
        
        # Draw outline
        cv2.polylines(img, [points_array], isClosed=True, color=color, thickness=2)
...
def extract_contours():
    ...
    try:
        ...
        result = {
            ...
            "metadata": {
                "date": "2025-05-29 22:15:52",  # Updated timestamp
...
@app.route('/contours/<job_id>', methods=['GET', 'PUT'])
def handle_contours(job_id):
    """Get or update the extracted contours for a job"""
    # Validate job_id to prevent path traversal
    if not job_id or not all(c.isalnum() or c == '-' for c in job_id):
        return jsonify({"error": "Invalid job ID"}), 400
    ...
    if request.method == 'GET':
        ...
        contours_data['metadata']['date'] = "2025-05-29 22:15:52"
        ...
        # Update contours
        try:
            # Get the updated contours data from request
            ...
            if 'contours' not in updated_data:
                return jsonify({"error": "Invalid data format - missing contours"}), 400
            
            # Update metadata
            if 'metadata' not in updated_data:
                updated_data['metadata'] = {}
            
            updated_data['metadata']['date'] = "2025-05-29 22:15:52"
            updated_data['metadata']['user'] = "FETHl"
            updated_data['metadata']['modified'] = True
            
            # Save updated contours
            contours_path = os.path.join(app.config['RESULT_FOLDER'], f"{job_id}_contours.json")
            with open(contours_path, 'w') as f:
                json.dump(updated_data, f, cls=NumpyJSONEncoder)
            
            # Also update in editor folder if exists
            editor_contours_path = os.path.join(app.config['EDITOR_CONTOURS_FOLDER'], f"{job_id}_contours.json")
            if os.path.exists(editor_contours_path):
                with open(editor_contours_path, 'w') as f:
                    json.dump(updated_data, f, cls=NumpyJSONEncoder)
            
            logger.info(f"Updated contours for job {job_id}")
            ...
            logger.error(f"Error updating contours: {str(e)}")
            logger.error(traceback.format_exc())
            return jsonify({"error": str(e)}), 500

@app.route('/export/svg/<job_id>', methods=['GET'])
def export_svg(job_id):
    ...
    try:
        ...
        # Get image dimensions
        width = contours_data.get('image_size', {}).get('width', 800)
        height = contours_data.get('image_size', {}).get('height', 600)
        contours = contours_data.get('contours', [])
        
        # Generate SVG
        svg_content = generate_svg(contours, width, height)
        
        # Save SVG to exports folder
        svg_filename = f"{job_id}_contours.svg"
        svg_path = os.path.join(app.config['EXPORTS_FOLDER'], svg_filename)
        
        with open(svg_path, 'w', encoding='utf-8') as f:
            f.write(svg_content)
        
        logger.info(f"Generated SVG export for job {job_id}")
        
        # Return SVG file
        return send_file(svg_path, as_attachment=True, download_name=svg_filename, mimetype='image/svg+xml')
...
@app.route('/export/png/<job_id>', methods=['GET'])
def export_png(job_id):
    ...
    try:
        ...
        # Load original image if overlay is requested
        overlay_image = None
        if overlay:
            for ext in ['jpg', 'jpeg', 'png', 'bmp', 'tif', 'tiff']:
                image_path = os.path.join(app.config['UPLOAD_FOLDER'], f"{job_id}.{ext}")
        ...
        # Generate PNG
        png_image = generate_png_from_contours(contours, width, height, transparent, overlay_image)
        
        # Save PNG to exports folder
        png_filename = f"{job_id}_contours.png"
        png_path = os.path.join(app.config['EXPORTS_FOLDER'], png_filename)
        
        save_success = save_image_multiple_ways(png_image, png_path)
        
        if not save_success:
            return jsonify({"error": "Failed to save PNG export"}), 500
        
        logger.info(f"Generated PNG export for job {job_id}")
        
        # Return PNG file
        return send_file(png_path, as_attachment=True, download_name=png_filename, mimetype='image/png')
...
def regenerate_mask_from_contours_endpoint(job_id):
    ...
    try:
        ...
        data['metadata']['date'] = "2025-05-29 22:15:52"
        ...
        if os.path.exists(editor_contours_path):
            ...
            editor_data['metadata']['date'] = "2025-05-29 22:15:52"
...
if __name__ == '__main__':
    ...
    extractor.date = "2025-05-29 22:15:52"  # Updated with latest timestamp
